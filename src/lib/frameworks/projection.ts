...
/** Common shape across blocks and labels for the "does it occur on date" test. */
...
/** Whether a weekly/date/periodic item lands on the given yyyy-mm-dd. */
export function itemAffectsDate(item: RecurringItem, dateKey: string): boolean {
  if (!withinRange(dateKey, item.effective_from, item.effective_to)) return false;
  const d = fromDateKey(dateKey);
  if (item.scope === "weekly") return item.day_of_week === d.getDay();
  if (item.scope === "date") return item.specific_date === dateKey;
  if (item.scope === "monthly")
    return matchesPeriodic(item.month_anchor, item.month_interval, item.period_unit, d);
  return false;
}
...
function matchesPeriodic(
  ...
  // Hourly blocks recur every day (the per-day slots are generated by the
  // caller); the interval counts hours within the day, not days.
...
export function projectFrameworkBlocks(
  ...
  for (let cursor = new Date(from); toDateKey(cursor) <= toKey; cursor = addDays(cursor, 1)) {
    ...
    // periodic blocks (every N hours/days/weeks/months from the anchor)
    for (const m of monthly) {
      ...
      if (m.period_unit === "hour") {
        // Every N hours within the day → multiple slots from start_minute.
        const n = Math.max(1, m.month_interval ?? 1);
        const dur = Math.max(15, m.end_minute - m.start_minute);
        const occ = statusByKey.get(`${m.id}|${dateKey}`);
        for (let t = m.start_minute; t < 1440; t += n * 60) {
          const endT = Math.min(t + dur, 1440);
          out.push({
            id: `fwblock:${m.id}:${dateKey}:${t}`,
            frameworkId: framework.id,
            blockId: m.id,
            title: m.title,
            color: m.color ?? framework.color ?? null,
