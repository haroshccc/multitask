...
/**
 * Tall narrow stats widget: top picker to choose the dimension to slice by,
 * below it a vertical bar list `{label, count}` ordered by count descending.
 * Counts come from the FULL recordings set so the breakdown stays stable
 * regardless of the active filter — handy for steering filters.
 */
export function StatsTallWidget() {
  ...
  return (
    <div className="card h-full flex flex-col overflow-hidden">
      <header className="px-3 py-2 border-b border-ink-200 bg-ink-50/50 flex items-center gap-1.5">
        <BarChart3 className="w-3.5 h-3.5 text-ink-600" />
        <span className="text-xs font-medium text-ink-700">סטטיסטיקה</span>
      </header>
...
function byEmojiId(rows: NamedRow[]): (id: string) => string {
  const map = new Map(
    rows.map(
      (r) =>
        [r.id, `${visibleEmoji(r.emoji)}${r.name}`] as const,
    ),
  );
  return (id) => map.get(id) ?? "—";
}
    ...
    switch (dim) {
      ...
      case "recording_list": {
        ...
        if (!lists || lists.size === 0) {
          ...
          // Each recording can be in multiple lists — count once per list.
