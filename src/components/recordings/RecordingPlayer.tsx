...
export function RecordingPlayer({ recording }: Props) {
  const { data: url, isLoading, error } = useRecordingAudioUrl(recording.storage_path);
  ...
  return (
    <div className="card p-5 space-y-4">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 text-xs">
        ...
        <Meta label="שפה" value={recording.language} />
      </div>

      {/* Phase 6ב/ג placeholders --------------------------------------------- */}
      <section className="rounded-md border border-dashed border-ink-300 bg-ink-50 px-3 py-3">
        ...
        <p className="mt-1 text-xs text-ink-500 leading-relaxed">
          ...
          משימות) מתוכננים לפאזה 6ב/6ג. אחרי הקלטה, הסטטוס נשאר{" "}
